...
```{r pop_biden_3, echo=TRUE}
ggplot(data=covid_data) + geom_point(mapping=aes(size=covid_death_prop,
                                                 x=prop_biden_votes,
                                                 y=POP_ESTIMATE_2019),
                                     alpha=0.3) +
  scale_y_continuous(trans='log2') +
  geom_smooth(mapping=aes(y=POP_ESTIMATE_2019, 
                          x=prop_biden_votes)) +
  geom_hline(yintercept=mean(covid_data$POP_ESTIMATE_2019), 
             alpha=0.5) + 
  geom_vline(xintercept=mean(covid_data$prop_biden_votes),
             alpha=0.5) +
  labs(y="Population estimate 2019 (log-scale)",
       x="Proportion of votes for Joe Biden",
       title="Deaths by covid and election results",
       color="Proportion covid deaths")
```
...
### Proportion of white population, proportion of deaths due to COVID, and Biden
